...
function generateAnswerOptions(answer) {
  ...
  while (options.size < 5) {
    ...
    const candidate = Math.max(0, answer + spread * direction);
...
function animateQuestionIn() {
  ...
  gsap.fromTo(
    ...
    { y: 0, opacity: 1, scale: 1, duration: 0.28, stagger: 0.05, ease: "power2.out" }
  );
}

function animateCheckpointCard() {
  if (!window.gsap || REDUCED_MOTION) {
    return;
  }
